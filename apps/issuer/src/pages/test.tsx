...
class Issuer extends Component {
  ...
  componentDidMount(){
    document.title = "Ypiresia gia ton Issuer";
    this.callApi()
  ...
  //Issuer provides his owner address
  ...
  /* if we use backend server
    ...
    if (body == "error"){console.log("error");}
      ...
      this.setState( {
      txForIssuer: body.tranHash, contractAdd: body.contractAddress,
  ...
  */
  //the following code is for using the Blockchain Typescript Library
    ...
    this.setState( {
      txForIssuer: response.tranHash, contractAdd: response.contractAddress,
      willShowLoader: false, reject: false
    });
  }
  ...
  handleAwardv2 = async(event) => {
    ...
    */
    ...
    let response = await publishAward({
      ...
      contractAddressUsedByIssuer: cAdd
    });
    console.log(response);
    this.setState( {
  ...
  handleProofv2 = async(event) => {
    ...
    */
    ...
    let response = await publishProof({
       contractAddressUsedByIssuer: cAdd, sReq: sReq, c: c, c2: c2, nirenc: nirenc, ev: ev
    });
    console.log(response);
    this.setState( {
  ...
  render() {
   let displayTxForIssuer = ((this.state.txForIssuer == "") ? (<p></p>) : (<p> Issuer your transaction receipt is {this.state.txForIssuer}</p>));
   ...
   //in return we should also add a tab where Issuer can see the Request published for him
   //data will be retrieved from the database. When a Holder publishes a request for proof from the website,
   //website will save the data. Another way is for the website actively listen to all events emited from an Issuer's contract.
   //If we have many Issuer we should listen from all contracts. So if a Holder publishes a request without using the website
   ...
   //smart contract is emited so that it can be saved and later it could be displayed to the Issuer through the website.
   ...
   return (
     <div className="MakeRequest">
          ...
          <label>
            Publish an award <br />Insert the Ethereum address of the contract that you use as an Issuer and the hash value of the award you want to publish.<br />
          ...
          <label>
            Publish a proof <br />Insert the Ethereum address of the contract that you use as an Issuer and all other necessary values.<br />
